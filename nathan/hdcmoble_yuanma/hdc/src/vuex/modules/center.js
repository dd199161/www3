...
const mutations = {
		...
		if (state.truename !== "" && state.bank_account !== "" && state.bank_password !== "") {
			axios({
				...
				if (response.data.code == 211028) {
					store.commit("centerTip", 5)//绑定银行卡失败
				} else if (response.data.code == 201027) {
					store.commit("centerTip", 6);//银行卡绑定成功
	...
	getEsWitFo(state) {
		...
		axios({
			...
			const sum_info = response.data.sum_info; //总数据
			const sum_data = response.data.data //获取扣除钱
