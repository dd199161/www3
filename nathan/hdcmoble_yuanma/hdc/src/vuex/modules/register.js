...
const mutations = {
	...
	loginout(state) {
		...
		store.state.navToggle=true;
		store.state.headToggle=true;
    VueCookie.delete('username',"password");
		store.state.navs.forEach(nav=> {
		...
		axios({
		  ...
		  params:{username:context.state.Globalusername,password:context.state.Globalpassword}
			...
			if (response.data.code == 211003) {
				context.commit("registerTip","用户名或密码错误");
			}else if (response.data.code == 201001) {
				context.commit("registerTip","登录成功");
				...
				//获取关闭的保存数据
				//联系客服打开
				context.state.contact_toggle=true;  //快捷支付打开
		...
		if (context.state.tranforMoney !== "") {
			axios({
				...
				if (response.data.code == 113020) {
					context.commit("registerTip","转账失败，请重试！");
