...
const userStore = new Vuex.Store({
      ...
      const index=value.index
		...
		swiperTopage(state, value) {
			if (value == 0) {
				...
				userStore.commit('registerTip',"请先登录");
			}
		},
	...
	actions:{
		...
		getStatusMaintain(context){
     axios({
			 methods:'get',
			  url:`${context.state.url}dist/php/mobile/mobile_platform_status.php`,
			 headers:context.state.register.headers,
			 timeout:100000,
		 }).then((res)=>{
			 res.data.info.forEach(item=>{
				 context.state.maintainStatus.set(item.tags,item.status);
			 })
		 }).catch((err)=>{
			 context.commit('centerTip',24);
			 setTimeout(()=>{context.commit('loginout')},1500);
