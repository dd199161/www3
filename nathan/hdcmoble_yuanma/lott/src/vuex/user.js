...
const userStore = new Vuex.Store({
	...
	mutations: {
		TestAndIos(state){
			state.TestAndIos=navigator.userAgent.toLowerCase();
		},
		swiper(state, value) {
			state.swiper = value;
		},
      ...
      const index=value.index
		...
		togglemain(state, value) {
			...
			//交易记录
			if (value == 6) {
				state.page = 1; //重置page页数
				state.centerTitle = "交易记录"
				state.status = true
				Vue.set(state.centers[5], 'toggle', true)
				state.select_1 = "类别:";
				state.select2 = false; //第二个关掉
				state.recordDatas = []; //进行清除
				//进行数据清空
	...
	actions: {
		...
		recordData(context) {
			//获取record数据
				var url;
				var page = context.state.page++; //页数进行加
				...
				if (url !== undefined && page <= context.state.totalNumber){
					...
					axios({
						...
						context.state.layer.main_layer = false;//获取数据
						setTimeout(()=>{context.state.layer.loging_status = false;},100);
						if(response.data.dataList==''){
							setTimeout(()=>{context.commit('centerTip', 20);},100);
						}else if(response.data.dataList!==''){
							let data_1 = response.data.data; //对于交易记录和转账记
							let data_2 = response.data.dataList; //对于投注记录
							let data_page = response.data.pagation;
							...
							if (data_page.totalNumber !== 0 && context.state.select2 == false) {
								...
								setTimeout(()=>{context.commit('centerTip', 20);},400);
							}
           }
					 //then结束
				 }).catch((err)=>{
					 context.commit('centerTip',24);
		       setTimeout(()=>{context.commit('loginout')},1500);
				 })
		...
		loginGame(context,value){
			const loginStatus=context.state.register.loginStatus;
			if(loginStatus===211003&&value!=='mg_game'){
				 window.location.href=`${context.state.url}dist/php/mobile/mobile_game_login.php?action=${value}&username=${context.state.register.Globalusername}&oid=${context.state.register.oid}`;
