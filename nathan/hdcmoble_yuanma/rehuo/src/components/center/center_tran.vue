...
.transform
 .ctranT
  ...
  span(style="margin-left:0.6rem;") 余额 :
   ...
   |  元
  v-touch.ctraf(tag="span",@tap="refreshMoney")
    ...
    li(v-for="(item,$index) in $store.state.register.choices")
     ...
     p
      v-touch(tag="span",@tap="getEachMoney($index)")
       icon(name="refresh")
  input.t_input(style="width:10rem;margin:0 auto;margin-bottom:0.7rem;",v-model="$store.state.register.tranforMoney",placeholder="请输入转账金额",type="text")
